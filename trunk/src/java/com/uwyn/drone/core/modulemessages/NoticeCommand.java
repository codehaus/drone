...
public class NoticeCommand implements ModuleMessage
{
	private ServerMessage	mServerMessage = null;
	private String			mCommand = null;
	private String			mArguments = null;
	
	public NoticeCommand(ServerMessage serverMessage, String command, String arguments)
	{
	...
	public void execute(Bot bot, Module module)
	{
		try
		{
			module.noticeCommand(bot, mServerMessage.getPrefix().getNickName(), mCommand, mArguments, mServerMessage);
		}
		catch (Throwable e)
		{
			Logger.getLogger("com.uwyn.drone.core").severe("Error during module execution of notice command '"+mCommand+"' : "+ExceptionUtils.getExceptionStackTrace(e));
		}
	}
}
